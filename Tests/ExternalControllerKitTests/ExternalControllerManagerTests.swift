...
final class ExternalControllerManagerTests: XCTestCase {
    ...
    func testListeningAcceptsOnlySelectedDevice() async {
        await MainActor.run {
            ...
            provider.send(ButtonEvent(deviceId: "pad2", buttonId: "A", isPressed: true))
            XCTAssertEqual(controller.allMappings(), [])
            XCTAssertEqual(controller.state, .listening(actionId: "action.jump"))

            provider.send(ButtonEvent(deviceId: "pad1", buttonId: "A", isPressed: true))
            XCTAssertEqual(controller.mapping(for: "action.jump", deviceId: "pad1"), Mapping(deviceId: "pad1", buttonId: "A", actionId: "action.jump"))
    ...
    func testRuntimeInputGateBlocksTriggersWhenDisabled() async {
        await MainActor.run {
            let provider = TestProvider(kind: .keyboard, devices: [Device(id: "keyboard", name: "Keyboard", kind: .keyboard)])
            let controller = Self.makeController(provider: provider)
            controller.assign(deviceId: "keyboard", buttonId: "KEY_A", actionId: "action.jump")
            var triggered: [(String, String, String)] = []
            let observation = controller.observe(onActionTriggered: { triggered.append(($0, $1, $2)) })
            defer { observation.invalidate() }

            provider.send(ButtonEvent(deviceId: "keyboard", buttonId: "KEY_A", isPressed: true))
    ...
    func testPerDeviceMappingsStayIndependent() async {
        await MainActor.run {
            ...
            controller.assign(deviceId: "pad1", buttonId: "A", actionId: "action.jump")
            controller.assign(deviceId: "pad2", buttonId: "A", actionId: "action.jump")

            XCTAssertEqual(controller.mappings(for: "pad1"), [Mapping(deviceId: "pad1", buttonId: "A", actionId: "action.jump")])
            XCTAssertEqual(controller.mappings(for: "pad2"), [Mapping(deviceId: "pad2", buttonId: "A", actionId: "action.jump")])
        }
    }
    ...
    func testMappingConflictReplacementIsPerDevice() async {
        await MainActor.run {
            ...
            controller.assign(deviceId: "midi1", buttonId: "NOTE_60", actionId: "action.jump")
            controller.assign(deviceId: "midi1", buttonId: "NOTE_61", actionId: "action.jump")
            controller.assign(deviceId: "midi1", buttonId: "NOTE_61", actionId: "action.pause")

            XCTAssertEqual(controller.allMappings(), [Mapping(deviceId: "midi1", buttonId: "NOTE_61", actionId: "action.pause")])
        }
    }
    ...
    func testRefreshPreservesMappingsAndReusesProvider() async {
        await MainActor.run {
            ...
            XCTAssertEqual(controller.mapping(for: "action.jump", deviceId: "keyboard")?.buttonId, "KEY_A")
        }
    }
